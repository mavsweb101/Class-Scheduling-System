...
include('../files/includes/dbcon.php');
	...
	foreach ($first as $daym){
		...
		natural join member natural join time where member_id='$member' and exam_sched.time_id='$daym' and day='first' and settings_id='$sid'")or die(mysqli_error($con));
		...
		natural join member natural join time where room='$room' and exam_sched.time_id='$daym' and day='first' and settings_id='$sid'")or die(mysqli_error($con));
		...
		natural join member natural join time where cys='$cys' and exam_sched.time_id='$daym' and day='first' and settings_id='$sid'")or die(mysqli_error($con));
		...
		{
			mysqli_query($con,"INSERT INTO exam_sched(time_id,day,member_id,subject_code,cys,cys1,room,settings_id,encoded_by) 
				VALUES('$daym','first','$member','$subject','$cys','$cys1','$room','$set_id','$id')")or die(mysqli_error());
	...
	foreach ($second as $daym){
		//check conflict for member
		$query_member=mysqli_query($con,"select *,COUNT(*) as count from exam_sched 
		natural join member natural join time where member_id='$member' and exam_sched.time_id='$daym' and day='second' and settings_id='$sid'")or die(mysqli_error($con));
			$row=mysqli_fetch_array($query_member);
			$count_t=$row['count'];
			$time1=date("h:i a",strtotime($row['time_start']))."-".date("h:i a",strtotime($row['time_end']));
			$member1=$row['member_last'].", ".$row['member_first'];
		...
		//check conflict for room
		$query_room=mysqli_query($con,"select *,COUNT(*) as count from exam_sched 
		natural join member natural join time where room='$room' and exam_sched.time_id='$daym' and day='second' and settings_id='$sid'")or die(mysqli_error($con));
			$rowr=mysqli_fetch_array($query_room);
			$count_r=$rowr['count'];
			$timer=date("h:i a",strtotime($rowr['time_start']))."-".date("h:i a",strtotime($rowr['time_end']));
			$roomr=$rowr['room'];
			...
			//check conflict for class
		$query_class=mysqli_query($con,"select *,COUNT(*) as count from exam_sched 
		natural join member natural join time where cys='$cys' and exam_sched.time_id='$daym' and day='second' and settings_id='$sid'")or die(mysqli_error($con));
			$rowc=mysqli_fetch_array($query_class);
			$count_c=$rowc['count'];
			$cysc=$rowc['cys'];
			$timec=date("h:i a",strtotime($rowc['time_start']))."-".date("h:i a",strtotime($rowc['time_end']));
		...
		{
			mysqli_query($con,"INSERT INTO exam_sched(time_id,day,member_id,subject_code,cys,cys1,room,settings_id,encoded_by) 
				VALUES('$daym','second','$member','$subject','$cys','$cys1','$room','$set_id','$id')")or die(mysqli_error());
	...
	foreach ($third as $daym){
		//check conflict for member
		$query_member=mysqli_query($con,"select *,COUNT(*) as count from exam_sched 
		natural join member natural join time where member_id='$member' and exam_sched.time_id='$daym' and day='third' and settings_id='$sid'")or die(mysqli_error($con));
			$row=mysqli_fetch_array($query_member);
			$count_t=$row['count'];
			$time1=date("h:i a",strtotime($row['time_start']))."-".date("h:i a",strtotime($row['time_end']));
			$member1=$row['member_last'].", ".$row['member_first'];
		...
		//check conflict for room
		$query_room=mysqli_query($con,"select *,COUNT(*) as count from exam_sched 
		natural join member natural join time where room='$room' and exam_sched.time_id='$daym' and day='third' and settings_id='$sid'")or die(mysqli_error($con));
			$rowr=mysqli_fetch_array($query_room);
			$count_r=$rowr['count'];
			$timer=date("h:i a",strtotime($rowr['time_start']))."-".date("h:i a",strtotime($rowr['time_end']));
			$roomr=$rowr['room'];
			...
			//check conflict for class
		$query_class=mysqli_query($con,"select *,COUNT(*) as count from exam_sched 
		natural join member natural join time where cys='$cys' and exam_sched.time_id='$daym' and day='third' and settings_id='$sid'")or die(mysqli_error($con));
			$rowc=mysqli_fetch_array($query_class);
			$count_c=$rowc['count'];
			$cysc=$rowc['cys'];
			$timec=date("h:i a",strtotime($rowc['time_start']))."-".date("h:i a",strtotime($rowc['time_end']));
		...
		{
			mysqli_query($con,"INSERT INTO exam_sched(time_id,day,member_id,subject_code,cys,cys1,room,settings_id,encoded_by) 
				VALUES('$daym','third','$member','$subject','$cys','$cys1','$room','$set_id','$id')")or die(mysqli_error());
